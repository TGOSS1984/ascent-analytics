...
The more mature version of this setup would define the semantic model in a text format that *can* be version-controlled — **TMDL (Tabular Model Definition Language)**, letting measures, relationships, and table structures live as reviewable `.tmdl` files alongside the Python and SQL in this same repo, rather than trapped inside a binary file. That was originally scoped as "Tabular Editor + TMDL," on the assumption an external tool was required to get there; as of PBIP's GA in Power BI Desktop (Feb 2026), that's no longer true — native `File → Save As → Power BI Project (.pbip)` with TMDL storage does this without Tabular Editor at all. See [`docs/tmdl_exploration.md`](tmdl_exploration.md) for the full exploration, including what it does and doesn't actually fix (the rebuild-cascade problem above is a Power Query issue, not a file-format one, and TMDL doesn't solve it on its own).