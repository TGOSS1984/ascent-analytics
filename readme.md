...
- **The Power BI semantic model isn't version-controlled the way the code is.** This turned out to be a real, recurring source of friction over the course of the project — see [`docs/powerbi_lessons_learned.md`](docs/powerbi_lessons_learned.md) for a full account of what a single schema change actually costs to rebuild, and why. **Explored** — see [`docs/tmdl_exploration.md`](docs/tmdl_exploration.md): native PBIP + TMDL support in Power BI Desktop (GA Feb 2026) lets the semantic model live as reviewable `.tmdl` text files alongside the Python and SQL, no external tool required. Not yet applied to `powerbi/ascent_analytics.pbix` in this repo — that's the concrete remaining step, with the conversion process and honest scope (what it does and doesn't fix) already documented.
