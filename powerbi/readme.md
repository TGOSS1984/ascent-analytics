...
This writes 17 CSVs to `powerbi/data_export/`: the 13 star-schema dimension/fact tables, plus 4 pre-aggregated summary tables (`Summary_CustomerLTV.csv`, `Summary_GuidePerformance.csv`, `Summary_RoutePerformance.csv`, `Summary_WeatherFlaggedCancellations.csv`) exported from the SQL views built in commit 9. These summary tables load as standalone tables, not related to anything else — they're for dropping straight into a visual (e.g. a customer LTV table) without re-deriving the calculation in DAX, since the SQL already did it once, correctly, and there's no reason to do it twice.
